...
- (void)onResourceChanged:(OFResource*)resource
{
    ...
    if (lastPlayedGameLabelRect.size.width == 0) lastPlayedGameLabelRect = lastPlayedGameLabel.frame;
	...
	if ([user online])
    {
        ...
        lastPlayedGameLabel.frame = CGRectMake(lastPlayedGameLabelRect.origin.x   + onlineLabel.frame.size.width + 5.0, lastPlayedGameLabelRect.origin.y,
                                               lastPlayedGameLabelRect.size.width - onlineLabel.frame.size.width - 5.0, lastPlayedGameLabelRect.size.height);
	} 
	else 
	{
        if ([user isLocalUser])
        ...
        lastPlayedGameLabel.frame = lastPlayedGameLabelRect;
	}
