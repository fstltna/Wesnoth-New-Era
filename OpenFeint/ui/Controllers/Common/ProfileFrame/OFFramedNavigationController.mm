...
static void _genRects(CGRect& bannerFrame, CGRect& frameFrame, CGRect& navBarFrame, CGRect& bottomFrame, OFFramedNavigationControllerVisibilityFlags v, float keyboardHeight)
{
	// set the nav bar appropriately first.
	navBarFrame.origin.y = v.showNavBar ? 0.f : -navBarFrame.size.height;
	...
	bannerFrame.origin.y -= v.showBanner ? 0.f : bannerFrame.size.height;
	...
	frameFrame.origin.y -= v.showBanner ? kfBannerOverlap : 0.f;  // we want them overlapping slightly
	
	frameFrame.size.height -= frameFrame.origin.y;
	
	frameFrame.size.height -= keyboardHeight;
...
- (void)_repositionViews
{
	...
	_genRects(bannerFrame, frameFrame, navBarFrame, bottomFrame,
			  visibilityDoes, isKeyboardShown ? keyboardHeight : 0.f);
...
- (UIEdgeInsets)_calculateInsetsFor:(UIViewController*)viewController
{
	...
	_genRects(bannerFrame, frameFrame, navBarFrame, bottomFrame,
			  v, isKeyboardShown ? keyboardHeight : 0.f);
