/* $Id: formula_ai.cpp 34762 2009-04-12 09:38:14Z dragonking $ */
/*
...
variant formula_ai::get_value(const std::string& key) const
{
	...
	} else if(key == "recruits_of_side")
	{
		...
		for( size_t i = 0; i<get_info().teams.size(); ++i)
		{
			...
			const std::set<shared_string>& recruits = get_info().teams[i].recruits();
			if(recruits.size()==0)
				continue;
			for(std::set<shared_string>::const_iterator str_it = recruits.begin(); str_it != recruits.end(); ++str_it)
			{
