/* $Id: addon_management.cpp 35863 2009-05-24 09:54:22Z soliton $ */
/*
...
std::vector<shared_string> available_addons()
{
	std::vector<shared_string> res;
	std::vector<shared_string> files, dirs;
	const std::string parentd = get_addon_campaigns_dir();
	get_files_in_dir(parentd,&files,&dirs);

	for(std::vector<shared_string>::const_iterator i = dirs.begin(); i != dirs.end(); ++i) {
...
std::vector<shared_string> installed_addons()
{
	std::vector<shared_string> res;
	const std::string parentd = get_addon_campaigns_dir();
	std::vector<shared_string> files, dirs;
...
static void archive_dir(const std::string& path, const std::string& dirname, config& cfg, std::pair<std::vector<std::string>, std::vector<std::string> >& ignore_patterns)
{
	...
	for(std::vector<shared_string>::const_iterator i = files.begin(); i != files.end(); ++i) {
		bool valid = true;
	...
	for(std::vector<shared_string>::const_iterator j = dirs.begin(); j != dirs.end(); ++j) {
		bool valid = true;
...
namespace {
	 ...
	 */
	void prepare_addons_list_for_display(std::vector<shared_string>& files,
	                                     std::vector<shared_string>& dirs,
	                                     const std::string& parent_dir)
	{
		std::vector<shared_string>::iterator i = files.begin();
		while(i != files.end())
		{
			std::string tempI = *i;
			//const size_t pos = i->rfind(".cfg", i->size());
			const size_t pos = tempI.rfind(".cfg", tempI.size());
			if(pos == std::string::npos) {
				...
				for(std::vector<shared_string>::iterator j = dirs.begin(); j != dirs.end() ; ++j) {
					if (*i == *j) {
						dirs.erase(j);
						break;
					}
				...
				std::replace(tempI.begin(), tempI.end(), '_', ' ');
				*i = tempI;
				i++;
			}
		}
	...
	{
		const std::vector<shared_string>& installed = installed_addons();
	...
	void upload_addon_to_server(game_display& disp, const std::string& addon, network::connection sock)
	{
		...
		if(!sock) {
			...
			symbols["error"] = (*data.child("error"))["message"];
		...
		if(!sock) {
			...
			gui::show_error_message(disp, ("The server responded with an error: \"") +
	                       ...
	                       bool* do_refresh)
	{
		...
		const std::vector< shared_string >& all_local = installed_addons();
		...
		const std::vector< shared_string >& all_publish = available_addons();
		...
		foreach(config* const remote_addon, remote_addons_list) {
			...
			std::vector<shared_string>::const_iterator local_match =
		...
		// column contents
		std::vector<shared_string> addons, titles, oldversions, newversions, options, filtered_opts;
		std::vector<int> sizes;

		std::vector<shared_string> types, uploads;
...
// 		index = filter->get_index(index);
	...
	void download_addons(game_display& disp, std::string remote_host, bool update_mode, bool* do_refresh)
	{
		...
		try {
			...
			// column contents
			std::vector<shared_string> addons, titles, versions, uploads, types, options, options_to_filter;
			std::vector<int> sizes;

			std::string sep(1, COLUMN_SEPARATOR);
			const std::string& heading =
#ifdef __IPHONEOS__
			(formatter() << HEADING_PREFIX << sep << _("Name") << sep
			  << _("Type") << sep << _("Downloads") ).str();
#else
			...
			const std::vector< shared_string >& publish_options = available_addons();

			std::vector< shared_string > delete_options;

			foreach(const config* i, addon_cfgs) {
				const config& c = *i;

				const std::string& name = c["name"];
				const std::string& downloads = c["downloads"];
				const std::string& size = c["size"];
				//const std::string& sizef = format_file_size(size);
	...
	{
		std::vector<shared_string> addons;
		std::vector<shared_string> addon_dirs;
...
void refresh_addon_version_info_cache()
{
	...
	const std::vector<shared_string>& addons = installed_addons();
	if(addons.empty()) {
