/* $Id: statistics_dialog.cpp 33515 2009-03-10 08:37:02Z silene $ */
/*
...
namespace {
std::vector<shared_string> create_unit_table(const statistics::stats::str_int_map& m, unsigned int team)
{
	std::vector<shared_string> table;
...
void statistics_dialog::action(gui::dialog_process_info &dp_info)
{
	...
	std::string title;
	std::vector<shared_string> items_sub;
	switch(result()) {
...
void statistics_dialog::make_damage_line(std::vector<shared_string>& items,
