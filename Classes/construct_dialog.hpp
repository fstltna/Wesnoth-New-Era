/* $Id: construct_dialog.hpp 31859 2009-01-01 10:28:26Z mordante $ */
/*
...
public:
	filter_textbox(CVideo& video, const std::string& header,
			const std::vector<shared_string>& items,
			const std::vector<shared_string>& items_to_filter, size_t header_row,
...
private:
	std::vector<shared_string> items_, items_to_filter_, filtered_items_;
	std::vector<int> index_map_;
	std::vector<shared_string> last_words;
	size_t header_row_;
	gui::dialog& dialog_;
...
public:
	...
	void set_menu(const std::vector<shared_string> & menu_items, menu::sorter* sorter=NULL);
	void set_menu_items(const std::vector<shared_string> &menu_items);
