/* $Id: filesystem.cpp 34025 2009-03-22 12:39:12Z mordante $ */
/*
...
void get_files_in_dir(const std::string directory,
					  std::vector<shared_string>* files,
...
std::string get_cache_dir()
{
	...
	const std::string dir_path = game_config::path + "/precache";
	return get_dir(dir_path);
}
...
#ifndef __AMIGAOS4__
  #ifdef __IPHONEOS__
	game_config::preferences_dir = home + std::string("/Documents/") + path;
  #else
...
std::string read_file(const std::string &fname)
{
	...
	// KP: avoid using file streams on iPhone
...
std::string next_filename(const std::string &dirname, unsigned int max)
{
	...
	std::vector<shared_string>::reverse_iterator i;
...
#ifndef __IPHONEOS__
		// KP: do not calculate checksums on iPhone, because data can't be modified anyways
...
#ifdef __IPHONEOS__
	// not found.... check again and replace .ogg/.wav with .caf (for sound effects)
	pos = filename.rfind(".ogg");
	if (pos == std::string::npos)
		 pos = filename.rfind(".wav");
	if (pos != std::string::npos) 
	{
		std::string nf = filename.substr(0, pos);
		nf += ".caf";
		foreach (const std::string &path, get_binary_paths(type))
		{
			const std::string file = path + nf;
			DBG_FS << "  checking '" << path << "'\n";
			if(file_exists(file)) {
				DBG_FS << "  found at '" << file << "'\n";
				return file;
		...
		// ok, try again with .mp3 (for music)
		nf = filename.substr(0, pos);
		nf += ".mp3";
		foreach (const std::string &path, get_binary_paths(type))
		{
			const std::string file = path + nf;
