/* $Id: variable.hpp 33100 2009-02-26 01:34:22Z sapient $ */
/*
 ...
 */
	...
	*/
	const std::string operator[](const std::string& key) const { return expand(key); }
	const std::string expand(const std::string&) const; /** < Synonym for operator[] */
	//const std::string& get_attribute(const std::string& key) const { return (*cfg_)[key]; }
	const std::string get_attribute(const std::string& key) const { return (*cfg_)[key]; }
	//const shared_string& get_attribute(const std::string& key) const { return (*cfg_)[key]; }
	...
	struct all_children_iterator {
		...
		typedef std::pair<const shared_string, const vconfig> value_type;
...
public:
	...
	shared_string key; //the name of the internal attribute or child
