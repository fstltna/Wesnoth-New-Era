/* $Id: sdl_utils.hpp 34753 2009-04-12 06:01:26Z boucman $ */
/*
...
// KP: added texture cache functions
...
private:
	...
	{
		if (surf != NULL /*&& surf != SDL_GetVideoSurface()*/)
			++surf->refcount;
	}
	...
	struct free_sdl_surface {
		void operator()(SDL_Surface *surf) const
		{
			...
			{
				if (surf->refcount <= 1)
					free_cache_texture(surf);
				SDL_FreeSurface(surf);		// KP: decreases refcount
			}
		}
	};
...
public:
	...
	{
//		SDL_Surface *oldSurf = surface_.get();
//		if (oldSurf != NULL)
//			free_cache_texture(oldSurf);		// the current surface changed, so invalidate cache
		SDL_Surface *surf = o.surface_.get();
		sdl_add_ref(surf); // need to be done before assign to avoid corruption on "a=a;"
		surface_.assign(surf);	// KP: assigning always releases the old resource, and replaces it with the new one
	}
...
// KP: remastered for openGL CVideo support
void getClipRect( SDL_Rect *rect);
void setClipRect(const SDL_Rect *rect);

struct clip_rect_setter
{
	clip_rect_setter(const SDL_Rect& r) : rect()
	{
		getClipRect(&rect);
		setClipRect(&r);
	}
	
	~clip_rect_setter() { setClipRect(&rect); }
