/* $Id: team.cpp 33693 2009-03-15 17:46:14Z jhinrichs $ */
/*
...
team::team_info::team_info(const config& cfg) :
		...
		can_recruit(),
		global_recruitment_pattern(utils::split_shared(cfg["global_recruitment_pattern"])),
		recruitment_pattern(utils::split_shared(cfg["recruitment_pattern"])),
		enemies(),
		team_name(sgettext(cfg["team_name"].c_str())),
		user_team_name(sgettext(cfg["user_team_name"].c_str())),
		save_id(cfg["save_id"]),
...
//		user_team_name = user_team_name.from_serialized(user_team_name);
	...
	{
		recruitment_pattern =
			utils::split_shared(global_ai_params["recruitment_pattern"]);
...
void team::team_info::write(config& cfg) const
{
	...
	for(std::set<shared_string>::const_iterator cr = can_recruit.begin(); cr != can_recruit.end(); ++cr) {
	...
	for(std::vector<shared_string>::const_iterator p = global_recruitment_pattern.begin();
	...
	for(std::vector<shared_string>::const_iterator p = recruitment_pattern.begin();
...
void team::set_recruits(const std::set<shared_string>& recruits)
{
	info_.can_recruit = recruits;
	info_.average_price = 0;
}

void team::add_recruits(const std::set<shared_string>& recruits)
{
...
void team::set_time_of_day(int turn, const time_of_day& tod)
{
	...
	info_.recruitment_pattern = utils::split_shared(aiparams_["recruitment_pattern"]);
...
void team::log_recruitable(){
	...
	for (std::set<shared_string>::const_iterator it = info_.can_recruit.begin();
