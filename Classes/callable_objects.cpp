/* $Id: callable_objects.cpp 34673 2009-04-10 11:13:24Z dragonking $ */
/*
...
variant unit_callable::get_value(const std::string& key) const
{
	if(key == "x") {
		...
		const std::map<shared_string, shared_string>& states_map = u_.get_states();
...
variant unit_type_callable::get_value(const std::string& key) const
{
	if(key == "id") {
		...
		std::vector<shared_string> abilities = u_.get_ability_list();
		std::vector<variant> res;

		if (abilities.empty())
			return variant( &res );

		for (std::vector<shared_string>::iterator it = abilities.begin(); it != abilities.end(); ++it)
		{
			res.push_back( variant(*it) );
		}
		return variant( &res );
