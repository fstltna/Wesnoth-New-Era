/* $Id: animated.i 31858 2009-01-01 10:27:41Z mordante $ */
/*
...
// KP: added specialized functions for image::locator to support cache functions
...
//template<>
const image::locator animated<image::locator>::void_value_ = image::locator();

//template<>
animated<image::locator>::animated(int start_time) :
starting_frame_time_(start_time),
does_not_change_(true),
started_(false),
need_first_update_(false),
...
//template<>
animated<image::locator>::animated(const std::vector<std::pair<int,image::locator> > &cfg, int start_time, bool force_change ):
starting_frame_time_(start_time),
does_not_change_(true),
started_(false),
need_first_update_(false),
...
current_frame_key_(0)
{
	...
	//typename 
	std::vector< std::pair<int,image::locator> >::const_iterator itor = cfg.begin();
	for(; itor != cfg.end(); ++itor) {
...
//template<>
void animated<image::locator>::add_frame(int duration, const image::locator& value,bool force_change)
{
	if(frames_.empty() ) {
		does_not_change_=!force_change;
		frames_.push_back( frame(duration,value,starting_frame_time_));
...
//template<>
void animated<image::locator>::start_animation(int start_time, bool cycles)
{
	started_ = true;
	last_update_tick_ = current_ticks;
	acceleration_ = 1.0; //assume acceleration is 1, this will be fixed at first update_last_draw_time
...
//template<>
void animated<image::locator>::update_last_draw_time(double acceleration)
{
	if (acceleration > 0 && acceleration_ != acceleration) {
		int tmp = tick_to_time(last_update_tick_);
		acceleration_ = acceleration;
...
//template<>
bool animated<image::locator>::need_update() const
{
	if(need_first_update_) {
		return true;
	}
...
//template<>
bool animated<image::locator>::animation_finished_potential() const
{
	if(frames_.empty())
		return true;
	if(!started_ && start_tick_ == 0)
...
//template<>
bool animated<image::locator>::animation_finished() const
{
	if(frames_.empty())
		return true;
	if(!started_ && start_tick_ == 0)
...
//template<>
int animated<image::locator>::get_animation_time_potential() const
{
	if(!started_  && start_tick_ == 0 ) return starting_frame_time_;
...
//template<>
int animated<image::locator>::get_animation_time() const
{
	if(!started_  && start_tick_ == 0 ) return starting_frame_time_;
...
//template<>
int animated<image::locator>::get_animation_duration() const
{
	return get_end_time() - get_begin_time();
}

//template<>
const image::locator& animated<image::locator>::get_current_frame() const
{
	if(frames_.empty() )
		return void_value_;
	return frames_[current_frame_key_].value_;
...
//template<>
int animated<image::locator>::get_current_frame_begin_time() const
{
	if(frames_.empty() )
		return starting_frame_time_;
	return frames_[current_frame_key_].start_time_;
...
//template<>
int animated<image::locator>::get_current_frame_end_time() const
{
	if(frames_.empty() )
		return starting_frame_time_;
	return get_current_frame_begin_time() +get_current_frame_duration();
...
//template<>
int animated<image::locator>::get_current_frame_duration() const
{
	if(frames_.empty() )
		return 0;
	return frames_[current_frame_key_].duration_;
...
//template<>
int animated<image::locator>::get_current_frame_time() const
{
	if(frames_.empty() )
		return 0;
	//FIXME: get_animation_time() use acceleration but get_current_frame_begin_time() doesn't ?
...
//template<>
const image::locator& animated<image::locator>::get_first_frame() const
{
	if(frames_.empty() )
		return void_value_;
	return frames_[0].value_;
...
//template<>
const image::locator& animated<image::locator>::get_last_frame() const
{
	if(frames_.empty() )
		return void_value_;
	return frames_.back().value_;
...
//template<>
int animated<image::locator>::get_frames_count() const
{
	return frames_.size();
}

//template<>
int animated<image::locator>::get_begin_time() const
{
	return starting_frame_time_;
}

//template<>
int animated<image::locator>::time_to_tick(int animation_time) const
{
	if(!started_ && start_tick_ == 0) return 0;
	return start_tick_ + static_cast<int>((animation_time-starting_frame_time_)/acceleration_);
}
//template<>
int animated<image::locator>::tick_to_time(int animation_tick) const
{
	if(!started_ && start_tick_ == 0) return 0;
	return static_cast<int>(
							(static_cast<double>(animation_tick - start_tick_) *
...
//template<>
int animated<image::locator>::get_end_time() const
{
	if(frames_.empty())
		return starting_frame_time_;
	return frames_.back().start_time_ + frames_.back().duration_;
...
//template<>
void animated<image::locator>::remove_frames_until(int new_starting_time)
{
	while (starting_frame_time_  < new_starting_time && !frames_.empty() )  {
		starting_frame_time_ += frames_[0].duration_;
		frames_.erase(frames_.begin());
...
//template<>
void animated<image::locator>::remove_frames_after(int new_ending_time)
{
	int last_start_time = starting_frame_time_;
	//typename 
	std::vector<frame>::iterator current_frame = frames_.begin();
	while (last_start_time  < new_ending_time && current_frame != frames_.end()) {
		last_start_time += current_frame->duration_;
		current_frame++;
	}
...
//template<>
void animated<image::locator>::set_begin_time(int new_begin_time)
{
	const int variation = new_begin_time - starting_frame_time_;
	starting_frame_time_ += variation;
	for(
		//typename 
		std::vector<frame>::iterator itor = frames_.begin(); itor != frames_.end() ; itor++) {
		itor->start_time_ += variation;
	}
}
