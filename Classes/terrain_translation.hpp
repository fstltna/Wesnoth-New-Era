/* $Id: terrain_translation.hpp 33687 2009-03-15 16:50:42Z mordante $ */
/*
...
namespace t_translation {
	...
	struct t_terrain {
		...
		void saveCache(FILE *file) const
		{
			...
			fwrite(&overlay, sizeof(overlay), 1, file);
		}
		
		void loadCache(MEMFILE *file, char *loadBuffer)
		{
			mread(&base, sizeof(base), 1, file);
			mread(&overlay, sizeof(overlay), 1, file);
		}
	};
	...
	struct t_match{
		...
		void saveCache(FILE *file) const
		{
			...
			for (int i=0; i < size; i++)
			{
				mask[i].saveCache(file);
			}
			
			size = masked_terrain.size();
			fwrite(&size, sizeof(size), 1, file);
			for (int i=0; i < size; i++)
			{
				masked_terrain[i].saveCache(file);
			}
		...
		void loadCache(MEMFILE *file, char *loadBuffer)
		{
			...
			terrain.reserve(size);
			for (int i=0; i < size; i++)
			{
				t_terrain ter;
				ter.loadCache(file, loadBuffer);
			...
			mread(&size, sizeof(size), 1, file);
			masked_terrain.reserve(size);
			for (int i=0; i < size; i++)
			{
				t_terrain ter;
				ter.loadCache(file, loadBuffer);
				masked_terrain.push_back(ter);
			}
			
			char bChar;
			mread(&bChar, sizeof(bChar), 1, file);
			has_wildcard = bChar != 0;
			mread(&bChar, sizeof(bChar), 1, file);
			is_empty = bChar != 0;
		}
	};
