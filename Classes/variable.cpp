/* $Id: variable.cpp 34564 2009-04-05 19:21:44Z silene $ */
/*
...
const std::string vconfig::expand(const std::string& key) const
{
	if(!cfg_) {
		return t_string();
	}
	//const t_string& val = (*cfg_)[key];
	const std::string& val = (*cfg_)[key];
	if(repos != NULL && !val.empty()) {
		std::string interp = utils::interpolate_variables_into_string(val, *repos);
		if(val != interp) {
...
std::pair<const shared_string,const vconfig> vconfig::all_children_iterator::operator*() const
{
	//return std::make_pair<const std::string, const vconfig>(get_key(), get_child());
	return std::make_pair<const shared_string, const vconfig>(get_key(), get_child());
}
...
vconfig::all_children_iterator::pointer vconfig::all_children_iterator::operator->() const
{
	...
	return pointer(new std::pair<const shared_string, const vconfig>(get_key(), get_child()));
}

//std::string vconfig::all_children_iterator::get_key() const
shared_string vconfig::all_children_iterator::get_key() const
{
	std::string key = i_.get_key();
	if(key == "insert_tag") {
	...
	vars(NULL)
{
	...
	std::string keyStr = key.get();
	const std::string::iterator index_start = std::find(keyStr.begin(),keyStr.end(),'[');
	explicit_index = index_start != keyStr.end();
	if(explicit_index) {
		const std::string::iterator index_end = std::find(index_start,keyStr.end(),']');
		const std::string index_str(index_start+1,index_end);
		...
		key = std::string(keyStr.begin(),index_start);
...
//t_string& variable_info::as_scalar() {
shared_string& variable_info::as_scalar() {
	assert(is_valid);
