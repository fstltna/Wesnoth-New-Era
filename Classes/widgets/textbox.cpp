/* $Id: textbox.cpp 33261 2009-03-01 18:12:59Z silene $ */
/*
...
void textbox::draw_contents()
{
	...
	if(text_image_ != NULL) {
		...
		blit_surface(dest.x, dest.y, text_image_);
	}
...
void textbox::handle_event(const SDL_Event& event)
{
	...
	int mousex, mousey;
	const Uint8 mousebuttons = SDL_GetMouseState(0, &mousex,&mousey);
	...
	if(clicked_inside) {
		set_focus(true);
		// KP: added auto keyboard support
	...
	// KP: iPhone sends delete key, not backspace as it should...
